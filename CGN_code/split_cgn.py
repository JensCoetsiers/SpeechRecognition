...
# The amount of padding we use. This padding is necessary for sorting in the import phase.
WIDTH = 3
...
def split_file(audio_dir, trans_dir, filename):
    ...
    for tau in root.iter("tau"):
        ...
        # # Apparently tostring does not return a string...
        xml_string = xml_string + ET.tostring(tau).decode("utf-8")
...
def split_audio(audio_dir, name, i, begin, end):
    ...
    # Save the new file
    # Use zfill to pad the index so we get 000 001 etc.
    new_name = name + "(" + str(i).zfill(WIDTH) + ")" + ".wav"
