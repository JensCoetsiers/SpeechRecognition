...
# This is used to parse the XML file with the transcriptions
import xml.etree.ElementTree as ET
import gzip          # This is used to extract the transcriptions if they are still zipped
from scipy.io import wavfile  # used to calculate audio duration (jensc)
...
def output_duration(length):
    ...
    some readable format and get the file duration
    '''
...
def process_language(audio_path, trans_path):
    # Needs to be sorted so that we can use the "previous" later.
    ...
    def maybe_add(possible_file, possible_filesize, possible_transcript):
        if possible_transcript and possible_transcript != "":
            print("File: " + possible_file +
                  " and Transcript: " + str(possible_transcript))
    ...
    for idx, file in enumerate(files):
        ...
        sample_rate, data = wavfile.read(final_path)
        ...
        if "(" not in file:  # The original file is still there but we only want to account for files that are split
            ...
            print(f"Too long Rejection: {file} and {seconds}")
            rejected += 1
        else:
            ...
            if possible_file is None:
                pass
            # When we start processing a new "big" file.
            elif file.endswith("(000).skp"):
                maybe_add(possible_file, possible_filesize,
                          possible_transcript)
                ...
                if idx == len(files) - 1:
                    ...
                    maybe_add(final_path, os.path.getsize(
                        final_path), transcript)
            else:
                print(f"Time Rejection: {possible_file} and {seconds}")
...
def generate_splits(data):
    ...
    with open(FILENAME_TRAIN, 'a') as f:
        ...
        train_data.to_csv(f, sep=',', mode='a', header=True,
                          index=False, encoding="ascii")

    print("Writing validation split")
    with open(FILENAME_DEV, 'a') as f:
        # We only want to write the header the first time, hence the quick check "header = ..."
        dev_data.to_csv(f, sep=',', mode='a', header=True,
                        index=False, encoding="ascii")

    print("Writing test split")
    with open(FILENAME_TEST, 'a') as f:
        # We only want to write the header the first time, hence the quick check "header = ..."
        test_data.to_csv(f, sep=',', mode='a', header=True,
                         index=False, encoding="ascii")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--components", nargs='+',
                        help="restrict the used data to a specified component")
    parser.add_argument(
        "--language", help="choose a single language for the model")
